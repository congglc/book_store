...
async function seedDatabase() {
  ...
  try {
    ...
    // Tạo dữ liệu địa chỉ
    console.log("Đang tạo dữ liệu địa chỉ...")
    const addresses = await seedAddresses(db, users)
    console.log(`Đã tạo ${addresses.length} địa chỉ!`)

    // Tạo dữ liệu đánh giá
    console.log("Đang tạo dữ liệu đánh giá...")
    const reviews = await seedReviews(db, users, books)
...
// Tạo dữ liệu địa chỉ cho user
async function seedAddresses(db, users) {
  const addresses = []
  for (const user of users.filter(u => u.role === "customer")) {
    addresses.push({
      userId: user._id,
  ...
  const result = await db.collection("addresses").insertMany(addresses)
  return Object.values(result.insertedIds).map((id, index) => ({
    ...addresses[index],
    _id: id,
  }))
...
// Tạo dữ liệu đánh giá cho sách
async function seedReviews(db, users, books) {
  const reviews = []
  for (const book of books) {
    ...
    for (let i = 0; i < reviewCount; i++) {
      ...
      } while (user.role !== "customer" || usedUserIds.includes(user._id.toString()))
      usedUserIds.push(user._id.toString())
      reviews.push({
        bookId: book._id,
        userId: user._id,
        rating: Math.floor(Math.random() * 5) + 1,
        comment: "Sách rất hay! " + book.title,
        createdAt: new Date(),
        updatedAt: new Date(),
      })
    }
  ...
  try {
    ...
    console.error("Lỗi khi insertMany reviews:", error)
    throw error
  }
}
